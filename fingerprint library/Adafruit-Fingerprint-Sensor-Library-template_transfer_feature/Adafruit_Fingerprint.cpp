...
uint8_t Adafruit_Fingerprint::image2Tz2(void) {
  SEND_CMD_PACKET(FINGERPRINT_IMAGE2TZ, 555);
}
...
/*!
    ...
    @returns <code>FINGERPRINT_PACKETRECIEVEERR</code> on communication error
*/
uint8_t Adafruit_Fingerprint::loadModelBuffer1(uint16_t location) {
  SEND_CMD_PACKET(FINGERPRINT_LOAD, 0x01, (uint8_t)(location >> 8),
                  (uint8_t)(location & 0xFF));
}

uint8_t Adafruit_Fingerprint::loadModelBuffer2(uint16_t location) {
...
uint8_t Adafruit_Fingerprint::matchTemplateBuffers(){
  SEND_CMD_PACKET(0x03);
}
...
/*!
   ...
   buffer to the UART
    ...
    @returns <code>FINGERPRINT_PACKETRECIEVEERR</code> on communication error
*/
uint8_t Adafruit_Fingerprint::getModel1(void) {
...
uint8_t Adafruit_Fingerprint::get_template_buffer(int bufsize, uint8_t ref_buf[]) { //new addition
  ...
  int i = 0;
  while (i<rcv_bt_len && (millis() - starttime) < 10000) {
...
uint8_t Adafruit_Fingerprint::getRawUART(int bufsize, uint8_t ref_buf[]){

  memset(ref_buf, 0xff, bufsize);

  uint8_t* tempBuffer = (uint8_t*)malloc(bufsize);

  memset(tempBuffer, 0xff, bufsize);
  ...
  memcpy(ref_buf, tempBuffer, bufsize);
  return FINGERPRINT_OK;

}

uint8_t Adafruit_Fingerprint::getTemplateUART(int bufsize, uint8_t ref_buf[]) {
  int total_len = bufsize + (11 * 16);  // Extra bytes due to headers/footers
  uint8_t tempBuffer[total_len];
  memset(tempBuffer, 0xff, total_len);  // Fill with 0xFF initially
  ...
  memset(ref_buf, 0xff, bufsize);  // Clear reference buffer before copying

  // **Extract valid data from each 139-byte chunk**
  int ref_buf_index = 0;
  for (int chunk = 0; chunk < 16; chunk++) {
      int tempBufferStart = chunk * 139;  // Start index of each 139-byte chunk

      // **Skip first 9 bytes and last 2 bytes, copy the next 128 bytes**
      for (int j = 9; j < 137; j++) {
          ref_buf[ref_buf_index++] = tempBuffer[tempBufferStart + j];
      }
  }
...
uint8_t Adafruit_Fingerprint::imageToUART(void) {
  SEND_CMD_PACKET(0x0A);
}


uint8_t Adafruit_Fingerprint::getImageUART(int bufsize, uint8_t ref_buf[]) {

  int total_len = bufsize + (11 * 100);  // Extra bytes due to headers/footers
  uint8_t* tempBuffer = (uint8_t*)malloc(total_len);
  memset(tempBuffer, 0xff, total_len);  // Fill with 0xFF initially
...
uint8_t Adafruit_Fingerprint::loadImageBuffer(void) {
  SEND_CMD_PACKET(0x0B);
  /*
...
*/
/**************************************************************************/
uint8_t Adafruit_Fingerprint::downloadModel1(void) { //new addition
  SEND_CMD_PACKET(FINGERPRINT_DOWNLOAD, 0x01);
}

uint8_t Adafruit_Fingerprint::downloadModel2(void) { //new addition
...
uint8_t Adafruit_Fingerprint::writeTemplateToSensorBuffer(int temp_Size, uint8_t ref_buf[]) {

  uint8_t data[128];
  Adafruit_Fingerprint_Packet packet(FINGERPRINT_DATAPACKET,128,data);

  for(int i = 0; i < 16; i++){
    if (i == 15) packet.type = FINGERPRINT_ENDDATAPACKET;
    memcpy(packet.data, ref_buf + (128 * i), 128);
    writeStructuredPacket(packet);
  }
...
boolean Adafruit_Fingerprint::writeImageToSensor(int temp_Size, uint8_t ref_buf[]){

  //if(loadImageBuffer() != FINGERPRINT_OK)return false;

  uint8_t data1[128];
  Adafruit_Fingerprint_Packet packet1(FINGERPRINT_DATAPACKET,128,data1);

  for(int i = 0; i < 100; i++){
    if (i == (100 - 1)) packet1.type = FINGERPRINT_ENDDATAPACKET;
    memcpy(packet1.data, ref_buf + (128 * i), 128);
    writeStructuredPacket(packet1);
  }
...
void Adafruit_Fingerprint::writeStructuredPacket(
  ...
  /*
  // **Override checksum if command is 0x0B**
  if (packet.data[0] == 0x0B) {
    Serial.println("---------");
    Serial.println("writing fixed sum");
    //uint16_t fixedSum = 0x000F;
    mySerial->write((uint8_t)0x00);  // **Fixed High Byte**
    mySerial->write((uint8_t)0x0F);  // **Fixed Low Byte**
  } else {
    mySerial->write((uint8_t)(sum >> 8));  // **Normal High Byte Checksum**
    mySerial->write((uint8_t)(sum & 0xFF));  // **Normal Low Byte Checksum**
  }
  */
