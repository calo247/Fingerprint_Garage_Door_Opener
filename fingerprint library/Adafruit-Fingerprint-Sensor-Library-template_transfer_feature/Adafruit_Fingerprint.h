...
public:
  ...
  uint8_t image2Tz2(void);
  uint8_t createModel(void);

  uint8_t emptyDatabase(void);
  uint8_t storeModel(uint16_t id);
  uint8_t loadModelBuffer1(uint16_t id);
  uint8_t loadModelBuffer2(uint16_t id);
  uint8_t matchTemplateBuffers();
  ...
  uint8_t downloadModel1(void); //new addiiton,for loading template data to buffer 
  uint8_t downloadModel2(void);
  boolean write_template_to_sensor(int temp_Size, uint8_t ref_buf[]) ; // new addition, for writing template data to sensor

  uint8_t writeTemplateToSensorBuffer(int temp_Size, uint8_t ref_buf[]);

  uint8_t getRawUART(int bufsize, uint8_t ref_buf[]);
  uint8_t getTemplateUART(int bufsize, uint8_t ref_buf[]);
  
  uint8_t imageToUART(void);
  uint8_t getImageUART(int bufsize, uint8_t ref_buf[]);

  boolean writeImageToSensor(int temp_Size, uint8_t ref_buf[]);

  uint8_t loadImageBuffer(void);
